...
class TestDifficultyScaling:
    ...
    def test_max_monsters_scaling_by_level(self):
        ...
        with patch('map_objects.game_map.from_dungeon_level') as mock_from_level:
            ...
            with patch('map_objects.game_map.randint', return_value=2), \
    ...
    def test_max_items_scaling_by_level(self):
        ...
        with patch('map_objects.game_map.from_dungeon_level') as mock_from_level:
            ...
            with patch('map_objects.game_map.randint', return_value=1), \
...
class TestMonsterSpawning:
    ...
    def test_orc_spawning_stats(self):
        ...
        with patch('map_objects.game_map.from_dungeon_level', return_value=3), \
             patch('map_objects.game_map.randint', return_value=1), \
             patch('map_objects.game_map.random_choice_from_dict', return_value='orc'):
    ...
    def test_multiple_monster_spawning(self):
        ...
        with patch('map_objects.game_map.from_dungeon_level', return_value=3), \
             patch('map_objects.game_map.randint', side_effect=[3, 1, 8, 8, 9, 9, 10, 10, 11, 11]):  # 3 monsters, 1 item, all positions
            
            with patch('map_objects.game_map.random_choice_from_dict', side_effect=['orc', 'troll', 'orc', 'healing_potion']):
                room = Mock()
                room.x1, room.x2 = 5, 15
                room.y1, room.y2 = 5, 15
                entities = []
                ...
                monsters = [e for e in entities if hasattr(e, 'fighter') and e.fighter is not None]
    ...
    def test_monster_position_collision_avoidance(self):
        ...
        with patch('map_objects.game_map.from_dungeon_level', return_value=2), \
             patch('map_objects.game_map.randint', side_effect=[2, 1, 10, 10, 11, 11, 12, 12]):  # Different positions
            
            with patch('map_objects.game_map.random_choice_from_dict', side_effect=['orc', 'troll', 'healing_potion']):
                room = Mock()
                room.x1, room.x2 = 5, 15
                room.y1, room.y2 = 5, 15
                entities = []
...
class TestDifficultyScalingIntegration:
    ...
    def test_complete_level_generation_with_scaling(self):
        ...
        player = Entity(0, 0, '@', (255, 255, 255), 'Player', blocks=True,
                       ...
                       inventory=inventory, level=level_comp)
        entities = [player]
        
        # Generate map with scaling
        game_map.make_map(max_rooms=10, room_min_size=4, room_max_size=8,
                         map_width=40, map_height=30, player=player, entities=entities)
        
        # Should have player + monsters + items + stairs
        assert len(entities) > 1
        
        # Check that we have a mix of entity types
        monsters = [e for e in entities if hasattr(e, 'fighter') and e.fighter is not None and e != player]
        items = [e for e in entities if hasattr(e, 'item') and e.item is not None]
        ...
        assert len(stairs) >= 1   # Should have at least one stairs (may have more due to room generation)
    ...
    def test_difficulty_scaling_affects_entity_distribution(self):
        ...
        for _ in range(10):
            ...
            with patch('map_objects.game_map.randint', side_effect=[1, 10, 10] * 20):  # Consistent positions
                room = Mock()
                room.x1, room.x2 = 5, 15
                room.y1, room.y2 = 5, 15
