...
- **Phase 14B – Render responsibility hardening** — **COMPLETE** (Dec 2025): libtcod isolated to IO renderers (`io_layer/*renderer*`, `render_functions.py`, entrypoints). Tooltips, death screen, menus/cutscenes now delegate to IO-layer renderers; render_optimization shimmed to IO. Tests: pytest, eco-ci, worldgen/portal sanity.
- **Phase 14C – Turn authority consolidation** — **COMPLETE** (Dec 2025): TurnStateAdapter is single authority for phase checks/transitions; AISystem and TurnController advance phases via adapter; GameState/TurnManager kept in sync; bot mode turn transitions verified. Tests: pytest, eco-ci, worldgen/portal sanity, bot turn transition suites.
